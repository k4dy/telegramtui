...
public class TelegramClient {
	...
	public TelegramClient(AppConfig config) {
		...
		Thread recv = new Thread(this::receiveLoop, "tdlib-receive");
		recv.setDaemon(true);
		recv.start();
	}
	...
	public static String getNestedType(String json, String objectField) {
	...
	private void receiveLoop() {
		while (!closed) {
			...
			String extra = getString(json, "@extra");
			if (extra != null) {
				Consumer<String> cb = callbacks.remove(extra);
				if (cb != null) {
					cb.accept(json);
					continue;
				}
			...
			if ("updateAuthorizationState".equals(getString(json, "@type"))) {
				String authType = getNestedType(json, "authorization_state");
	...
	private void sendParametersFlat() {
		...
		callbacks.put(extra, response -> {
			if ("error".equals(getString(response, "@type"))) {
